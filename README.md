...
3. Convert regular AutoCAD line entities to two-vertex 3D polylines.
4. Delete text and mtext at or below the configured tiny text height.
5. Delete text and mtext when its layer matches a configured delete text layer wildcard.
6. Leave text and mtext as-is when its layer matches a configured keep-as-text layer wildcard.
7. Convert remaining text and mtext annotation to mleaders with the arrowhead at the original text insertion point and the mleader text placed `15` drawing units northeast.
8. Flatten drafting annotation to elevation `0`, except block names matching the configured do-not-flatten wildcard list.
9. Set Civil 3D COGO point style and label style to `Standard` when those styles are available.
