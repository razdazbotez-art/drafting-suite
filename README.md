...
4. Leave text and mtext as-is when its layer matches a configured MLeader ignore layer wildcard.
5. Convert remaining text and mtext annotation to mleaders with the arrowhead at the original text insertion point and the mleader text placed `15` drawing units northeast.
6. Flatten drafting annotation to elevation `0`.
7. Set Civil 3D COGO point style and label style to `Standard` when those styles are available.
